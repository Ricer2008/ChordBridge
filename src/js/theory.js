...
//   beats[i] = { notes: [{degree,acc,octShift}], isRest, isTie, tokenIndex, raw }
// ------------------------------------------------------------
export function parseJianpu(text) {
  ...
  rawTokens.forEach((tok, ti) => {
    ...
    for (const part of parts) {
      if (part === "-") {
        beats.push({ notes: [], isRest: false, isTie: true, tokenIndex: ti, raw: tok });
        continue;
      }
      const subs = part.split("+").filter((s) => s.length);
      ...
      for (const sub of subs) {
        ...
        if (!m) {
          bad = { sub, reason: /[89]/.test(sub) ? "简谱只允许 0-7" : "非法记号（半音 #/b，八度 !/?，延音 -）" };
          break;
        }
        const deg = +m[2];
      ...
      beats.push({ notes: isRest ? [] : notes, isRest, isTie: false, tokenIndex: ti, raw: tok });
    }
  });
...
export function noteToPitch(note, key, globalOctave = 0) {
  ...
  return { midi, letter, spellAcc, octaveSci };
}
...
export function parseChordMarks(text, beatCount) {
  ...
  const re = /\[\s*(\d+)\s*:\s*([A-G][#b]?(?:maj7|m7|m|7|sus4|sus2|dim|aug|add9|6|9)?)\s*\]/g;
  let m, matchedLen = 0;
